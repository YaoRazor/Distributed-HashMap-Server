...
sub spawn_kvs {
  my $master = shift;
  my $port = shift;
  return spawn( "./kv_server", $master, $port );
}
...
sub start_nodes ($$){
  ...
  for (my $i = 0; $i < $n; $i++) {
		if ($command eq "kvs") {
			@pid = (@pid, spawn_kvs($p[0],$p[$i]));
			print "Start kv_server on $p[$i]\n";
...
print "Removing old logs and kill running kv_servers if there are any\n";
`rm -f *.log; killall kv_server`;

if ($do_run[0]) {
  print "test0: start 3 servers\n";
  start_nodes(3,"kvs");
  cleanup();
  sleep 2;
}
...
if ($do_run[1]) {
  print "test1: start 3 servers, kill third server\n";
  start_nodes(3,"kvs");
...
if ($do_run[2]) {
  print "test2: start 3 servers, kill first server\n";
  start_nodes(3,"kvs");
...
if ($do_run[3]) {
  ...
  print "test3: start 3 servers, kill a server, restart a server\n";
  start_nodes(3,"kvs");
...
if ($do_run[4]) {
  print "test4: 3 servers, kill third server, kill second server, restart third server, kill third server again, restart second server, re-restart third server, check logs\n";
  start_nodes(3,"kvs");
...
if ($do_run[5]) {
  print "test5: start 3 servers, send signal 1 to first server, kill third server, restart third server, check logs\n";
  start_nodes(3,"kvs");
...
if ($do_run[6]) {
  print "test6: start 4 servers, send signal 2 to first server, kill fourth server, restart fourth server, check logs\n";
  start_nodes(4,"kvs");
...
if ($do_run[7]) {
  print "test7: start 4 servers, send signal 2 to first server, kill fourth server, kill other servers, restart other servers, restart fourth server, check logs\n";
  start_nodes(4,"kvs");
  print "Sending paxos breakpoint 2 to first server on port $p[0]\n";
  spawn("./rsm_tester", $p[0]+1, "breakpoint", 4);
  sleep 3;
...
if ($do_run[8]) {
  print "test8: start 3 servers\n";
	start_nodes(3,"kvs");
  
  print "Start kv_tester $p[0]\n";
  $t = spawn("./kv_tester", $p[0]);

  print "   Wait for kv_tester to finish (waitpid $t)\n";
  waitpid_to($t, 600);

  if( system( "grep \"passed all tests successfully\" kv_tester-$p[0].log" ) ) {
    mydie( "Failed kv_tester for test 8" );
  }
...
if ($do_run[9]) {
  ...
  print "test9: start 3 servers, kill third one while kv tester is running\n";
	start_nodes(3,"kvs");
  ...
  if( system( "grep \"passed all tests successfully\" kv_tester-$p[0].log" ) ) {
    mydie( "Failed kv_tester for test 9" );
  }
...
if ($do_run[10]) {
  ...
  print "test10: start 3-process rsm, kill second slave and restarts it later while kv_tester is running\n";
	start_nodes(3,"kvs");
  ...
  print "Restarting killed kv_server on port $p[2]\n";
  $pid[2] = spawn_kvs($p[0], $p[2]);
  my @v5 = ($p[0],$p[1],$p[2]);
  wait_and_check_expected_view(\@v5);
  ...
  if( system( "grep \"passed all tests successfully\" kv_tester-$p[0].log" ) ) {
    mydie( "Failed kv_tester for test 10" );
  }
...
if ($do_run[11]) {
  ...
  print "test11: start 3-process rsm, kill primary while kv_tester is running\n";
  start_nodes(3,"kvs");
  ...
  if( system( "grep \"passed all tests successfully\" kv_tester-$p[0].log" ) ) {
    mydie( "Failed kv_tester for test 11" );
  }
...
if ($do_run[12]) {
  ...
  print "test12: start 3-process rsm, kill master at break1 and restart it while kv_tester is running\n";
  ...
  print "Restarting killed kv_server on port $p[0]\n";
  $pid[0] = spawn_kvs($p[1], $p[0]);
  ...
  if( system( "grep \"passed all tests successfully\" kv_tester-$p[0].log" ) ) {
    mydie( "Failed kv_tester for test 12" );
  }
...
if ($do_run[13]) {
  ...
  print "test13: start 3-process rsm, kill slave at break1 and restart it while kv_tester is running\n";
  ...
  if( system( "grep \"passed all tests successfully\" kv_tester-$p[0].log" ) ) {
    mydie( "Failed kv_tester for test 13" );
  }
...
if ($do_run[14]) {
  ...
  if( system( "grep \"passed all tests successfully\" kv_tester-$p[0].log" ) ) {
    mydie( "Failed kv_tester for test 14" );
  }
...
if ($do_run[15]) {
  ...
  if( system( "grep \"passed all tests successfully\" kv_tester-$p[0].log" ) ) {
    mydie( "Failed kv_tester for test 15" );
  }
...
if ($do_run[16]) {
  ...
  print "test16: start 3-process rsm, partition primary away from backups, heal it\n";
  ...
  if( system( "grep \"passed all tests successfully\" kv_tester-$p[0].log" ) ) {
    mydie( "Failed kv_tester for test 16" );
  }
